...
(function ($, window, document, undefined) {
    ...
    Zmenu.prototype.setStyles = function (ele) {
        ...
        this.mainNav.css({'border-right-color': this.op.borderDarkColor, 'float': this.op.floating});
        this.links.css({
            'height': this.op.height,
            'line-height': this.op.height,
        ...
        ;
        this.subMenu.css({'background-color': this.op.mainColor, 'width': this.op.subMenuWidth});
        this.subLinks.css({
            'color': this.op.fontColor,
            'font-size': this.op.fontSize,
        ...
        this.mobileSelect.fadeTo(0, 0).css({'height': this.op.height, 'line-height': this.op.height});
        this.mobileSpan.css({
            'line-height': this.op.height,
            'color': this.op.lablColor,
            'font-size': this.op.lablFontSize
        });
        this.mobileIcon.css({'color': this.op.iconColor, 'line-height': this.op.height, 'font-size': this.op.iconSize});
    ...
    Zmenu.prototype.createMobile = function (ele) {
        ...
        this.allLinks.each(function (indx, element) {
            var hrf = $(element).attr('href'), txt = $(element).text(), hclass = $(element).parents('.zlight-submenu').length, tr = '';
            if (hclass > 0) {
                for (i = 0; i < hclass; i++) {
                    tr += '-'
                }
            ...
            $(window).on('scroll.ZlightScrolling', function () {
                if ($(document).scrollTop() >= ele.op.transformBreak && ele.counter === 0) {
                    ele.element.css({
                        ...
                        'padding': '0',
                        'margin': '0'
                    }).stop(0, 0).animate({'top': '0'}, 300, 'swing');
                    ele.counter = 1
                } else if ($(document).scrollTop() < ele.op.transformBreak) {
                    ele.element.css({'position': 'relative', 'margin': '', 'padding': ''});
                    ele.counter = 0
                }
            })
        }
